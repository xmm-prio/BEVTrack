...
def getOffsetBB(box, offset, degrees=True, use_z=False, limit_box=True, inplace=False):
    ...
    if limit_box:
        if np.abs(offset[0]) > min(new_box.wlh[1], 3):
        ...
        if use_z and np.abs(offset[2]) > 0.5 * new_box.wlh[2]:
            offset[2] = np.random.uniform(0, 0.5 * new_box.wlh[2]) * np.sign(offset[2])
    if use_z:
...
def sync_augmentation(prev_pc, prev_box, this_pc, this_box, wlh_factor=1.25):
    prev_box_mask = nuscenes.utils.geometry_utils.points_in_box(prev_box, prev_pc.points, wlh_factor=wlh_factor)
    this_box_mask = nuscenes.utils.geometry_utils.points_in_box(this_box, this_pc.points, wlh_factor=wlh_factor)
    prev_in_box_pc = PointCloud(prev_pc.points[:, prev_box_mask])
    this_in_box_pc = PointCloud(this_pc.points[:, this_box_mask])
    rand_trans = np.random.uniform(low=-0.2, high=0.2, size=3)
    rand_trans[2] = 0.
    rand_rot = np.random.uniform(low=-5, high=5)
    flip_x, flip_y = np.random.choice([True, False], size=2, replace=True)
    prev_new_in_box_pc, prev_new_box = \
        apply_transform(prev_in_box_pc, prev_box, rand_trans, rand_rot, flip_x, flip_y)
    this_new_in_box_pc, this_new_box = \
        apply_transform(this_in_box_pc, this_box, rand_trans, rand_rot, flip_x, flip_y)

    prev_new_pc = copy.deepcopy(prev_pc)
    prev_new_pc.points[:, prev_box_mask] = prev_new_in_box_pc.points
    this_new_pc = copy.deepcopy(this_pc)
    this_new_pc.points[:, this_box_mask] = this_new_in_box_pc.points
    return prev_new_pc, prev_new_box, this_new_pc, this_new_box


def flip_augmentation(prev_points, this_points, this_bb):
    this_box = copy.deepcopy(this_bb)
    flip_y = np.random.choice([True, False])
    if flip_y:
        prev_points[:, 1] = -prev_points[:, 1]
        this_points[:, 1] = -this_points[:, 1]
        this_box.center[1] = -this_box.center[1]
        this_box.orientation = Quaternion(
            axis=[0, 0, -1], degrees=this_box.orientation.axis[-1] *
            this_box.orientation.degrees)

    return prev_points, this_points, this_box


def drop_points(prev_points, this_points):

    prev_num_points = prev_points.shape[0]
    this_num_points = this_points.shape[0]
    # num_keep = np.random.randint(min(50, prev_num_points, this_num_points),
    #                              min(400, prev_num_points, this_num_points) + 1, 1)
    if prev_num_points > 100 and this_num_points > 100:
        prev_new_pts_idx = np.random.choice(prev_num_points, size=prev_num_points // 4, replace=False)
        this_new_pts_idx = np.random.choice(this_num_points, size=this_num_points // 4, replace=False)
        return prev_points[prev_new_pts_idx, :], this_points[this_new_pts_idx, :]
    else:
        return prev_points, this_points
...
def movement_correct(points, offset_box_params):
    offset_center = offset_box_params[:, :3]
    offset_rot_angles = offset_box_params[:, -1]
    rot_mat_offset = rotz_batch_tensor(offset_rot_angles)
    points = torch.matmul(points, rot_mat_offset.transpose(1, 2))
...
def crop_pc_in_range(pc, sample_bb, point_cloud_range):
    ...
    new_pc.translate(trans)
    new_pc.rotate(rot_mat)
    x_filt_max = new_pc.points[0, :] < point_cloud_range[3]
    x_filt_min = new_pc.points[0, :] > point_cloud_range[0]
    y_filt_max = new_pc.points[1, :] < point_cloud_range[4]
    y_filt_min = new_pc.points[1, :] > point_cloud_range[1]
    z_filt_max = new_pc.points[2, :] < point_cloud_range[5]
    z_filt_min = new_pc.points[2, :] > point_cloud_range[2]
    close = np.logical_and(x_filt_min, x_filt_max)
    close = np.logical_and(close, y_filt_min)
    close = np.logical_and(close, y_filt_max)
    close = np.logical_and(close, z_filt_min)
    ...
    return PointCloud(new_pc.points[:, close])
